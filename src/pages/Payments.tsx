...
export function Payments() {
  ...
  const confirmDelete = async () => {
    if (deletingPayment) {
      try {
        const response = await deletePayment(deletingPayment.id);
        showSuccessMessage(
          getValue(response, "message") ||
            `Payment #${deletingPayment.id.slice(0, 8)} deleted successfully!`
        );
  ...
  useEffect(() => {
    if (paymentsData) {
      setAllPayments(getValue(paymentsData, "payments", []));
    }
  }, [paymentsData]);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="overflow-hidden">
        ...
        <Pagination
          ...
          total={getValue(paymentsData, "totalCount", 0)}
          pageCount={getValue(paymentsData, "totalPages", 0)}
      ...
      <DeleteModal
        ...
        description={
          deletingPayment
            ? `Are you sure you want to delete payment #${deletingPayment.id.slice(0, 8)}? This action cannot be undone.`
            : ""
        }
      />
    </div>
