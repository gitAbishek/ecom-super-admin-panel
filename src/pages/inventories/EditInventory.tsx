...
import { useGetSingleInventory, useUpdateInventory } from "../../hooks/inventories.hook";
...
export default function EditInventory() {
  ...
  const productOptions = useMemo(() => {
    return products.map(product => ({
      value: product._id,
      label: `${product.name} (${product.variants.length} variant${product.variants.length !== 1 ? 's' : ''})`
    }));
  }, [products]);
  ...
  // Populate form when inventory data loads
  useEffect(() => {
    if (inventory) {
      setSelectedProductId(inventory.productId);
      methods.setValue("productId", inventory.productId);
      methods.setValue("variantId", inventory.variantId);
      methods.setValue("quantity", inventory.quantity);
    }
  }, [inventory, methods]);
  ...
  const onSubmit: SubmitHandler<InventoryFormData> = async (data) => {
    if (!id) {
      showErrorMessage("Inventory ID is missing");
      return;
    }
    if (!data.productId) {
    ...
    try {
      const response = await updateInventory({ id, body: data });
      showSuccessMessage(getValue(response, "message") || "Inventory updated successfully!");
      navigate("/inventories");
    } catch (error) {
      showErrorMessage(getValue(error, "message") || "Failed to update inventory. Please try again.");
    }
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center space-x-4 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Update inventory for product variant
            </p>
          </div>
        </div>
        ...
        <form onSubmit={methods.handleSubmit(onSubmit)} className="space-y-6">
                ...
                {selectedProduct && (
                  <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      {selectedProduct.variants.length} variant{selectedProduct.variants.length !== 1 ? 's' : ''} available
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <CustomSearchableSelect
                    ...
                    onChange={(variantSku) => methods.setValue("variantId", variantSku)}
          ...
          <div className="flex items-center justify-end space-x-4 pt-6">
            ...
            <Button
              type="submit"
              disabled={isPending || !methods.watch("productId") || !methods.watch("variantId")}
